...
do
{
 ...
 //cout<<"\n\templo No.\t\tName \t\t Department \t\t Address";
 while(f)
 {
 t1.show();
 f.read((char*) &t1,(sizeof(t1)));
 ...
 break;
 case 3:
 cout<<"\nEnter the emplo number you want to find";
 cin>>rec;
 f.open("EmpRecord.txt",ios::in|ios::out);
 f.read((char*)&t1,(sizeof(t1)));
 while(f)
 {
 if(rec==t1.empNo)
 ...
 break;
 case 5:
 cout<<"\n\tEnter the employe number you want to modify";
 cin>>on;
 f.open("EmpRecord.txt",ios::in|ios::out);
 f.read((char*) &t1,(sizeof(t1)));
 while(f)
 ...
 case 7:
 int employ;
 cout<<"Please Enter the employ No. of Employee Whose Info You Want to Delete: ";
 cin>>employ;
 f.open("EmpRecord.txt",ios::in);
 ...
 while(!f.eof())
 {
 if (t1.getemployNo() != employ)
 g.write((char *)&t1,sizeof(t1));
 f.read((char *)&t1,sizeof(t1));
 }
 cout << "The record with the emplo no. " << employ << " has been deleted " << endl;
 f.close();
 g.close();
